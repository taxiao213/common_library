...
import com.taxiao.cn.commonlibrary.uitl.ui.PictureUtils;
...
public class CompressPicUtil {
    ...
    public static String getCompToRealPath(String strpath, int type) {
        ...
        try {
            if (strpath.contains("/")) {
                ...
                File compressImageFile = PictureUtils.createCompressImageFile(BaseApplication.getApplication());
                path = saveFile(rotaingBt, compressImageFile);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @param file 文件
     */
    public static String saveFile(Bitmap src, File file) {
        if (src == null) return "";
